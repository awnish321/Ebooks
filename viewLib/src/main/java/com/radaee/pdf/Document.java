...
public class Document
{
	...
	public int OpenMem( byte[] data, String password )
	{
		...
		{
			int ret = 0;
			hand_val = openMem( data, "ebook@rspl" );
			if( hand_val <= 0 && hand_val >= -10 )//error
			{
				ret = (int)hand_val;
				hand_val = 0;
	...
	public int OpenStream( PDFStream stream, String password )
	{
		...
		{
			int ret = 0;
			hand_val = openStream( stream, "ebook@rspl" );
			if( hand_val <= 0 && hand_val >= -10 )//error
			{
				ret = (int)hand_val;
				hand_val = 0;
	...
	{
		return encryptAs( hand_val, dst, upswd, "ebook@rspl", perm, method, id);
	}
	/**ebook@rspl
